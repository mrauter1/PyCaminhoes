...
def fazHtmlBarraHorizontal(porcentagem):
    ...
    return ploff.plot(fig, output_type="div")
...
def fazHtmlMedidor(porcentagem):
    ...
    return ('<div class="medidorInt1">'
            '  <style type = text/css>'
            '    .medidorInt1 {width: 100%; height: 270px; overflow: hidden; display: block;}'
            '  </style> '
            + ploff.plot(fig, output_type="div") +
            '</div>')